# Figure out which sorts of vector units we have
# Adapted from http://gitorious.org/vc/vc/blobs/master/cmake/OptimizeForArchitecture.cmake
...
macro(AddCompilerFlag _flag _cflags _cxxflags)
   ...
   if(${ARGC} EQUAL 4)
      set(${ARGV3} "${check_cxx_compiler_flag_${_flag_esc}}")
   endif(${ARGC} EQUAL 4)
endmacro(AddCompilerFlag)

macro(add_vector_flags _compiler _cflags _cxxflags)
   ...
   elseif(${_compiler} MATCHES "/(icpc|icc)$") # ICC
            ...
            if(_found)
               ...
               if(_found)
                  ...
                  if(_found)
                     ...
                     if(_found)
                        AddCompilerFlag("-xSSE2" ${_cflags} ${_cxxflags})
                     else(_found)
                        AddCompilerFlag("-xSSE3" ${_cflags} ${_cxxflags})
                     endif(_found)
                  else(_found)
   ...
   else(${_compiler} MATCHES "cl(.exe)?$")
      foreach(_flag ${_march_flag_list})
         AddCompilerFlag("-march=${_flag}" ${_cflags} ${_cxxflags} _good)
         if(_good)
            break()
         endif(_good)
      endforeach(_flag)
      foreach(_flag ${_enable_vector_unit_list})
         AddCompilerFlag("-m${_flag}" ${_cflags} ${_cxxflags})
      endforeach(_flag)
      foreach(_flag ${_disable_vector_unit_list})
         AddCompilerFlag("-mno-${_flag}" ${_cflags} ${_cxxflags})
      endforeach(_flag)
      ...
      #       if(NOT GCC_4_5_2)
      #          AddCompilerFlag("--param early-inlining-insns=12" ${_cflags} ${_cxxflags})
      #       endif(NOT GCC_4_5_2)
      #    endif(GCC_4_5_0)
      # endif(CMAKE_COMPILER_IS_GNUCXX)
   endif(${_compiler} MATCHES "cl(.exe)?$")
endmacro(add_vector_flags)
