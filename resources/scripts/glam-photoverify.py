...
for i,icenter in enumerate(table.bin_centers[idim]):
	...
	if len(splines) == 1:
        ...
        if three_d:
            ...
            for plot in range(ndim+offset,sample.shape[1],offset):
                plots.append(Gnuplot.Data(sample[:,xdim],
                                          sample[:,plot],
                                          title="%s CDF" % fit_labels[(plot - ndim - offset)/offset], axes='x1y1', with_='lines'))
		if timing and xdim == 3:
                	plots.append(Gnuplot.Data(sample[:,xdim],
                                          sample[:,plot+1],
                                          title="%s PDF" % fit_labels[(plot - ndim - offset)/offset], axes='x1y2', with_='lines lc 3'))
